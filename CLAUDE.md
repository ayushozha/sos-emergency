...
## Using the imported agent skills

The official Flutter and Dart skills are vendored into `.agents/skills/` (21 skills).
**Before doing a task one of them covers, read that skill's `SKILL.md` and follow its
steps** — they are the authoritative how-to. Where a skill conflicts with this file,
this file wins (e.g. JSON serialization is code-generated here, not manual).

Reach for a skill when the task matches:

| When you're about to… | Read this skill |
|---|---|
| Write/structure unit tests | `.agents/skills/dart-add-unit-test` |
| Create mocks for a dependency | `.agents/skills/dart-generate-test-mocks` (but prefer `mocktail`, below) |
| Measure test coverage | `.agents/skills/dart-collect-coverage` |
| Run analysis / apply fixes | `.agents/skills/dart-run-static-analysis` |
| Debug a runtime error | `.agents/skills/dart-fix-runtime-errors` |
| Resolve `pub` version conflicts | `.agents/skills/dart-resolve-package-conflicts` |
| Use switch expressions / pattern matching | `.agents/skills/dart-use-pattern-matching` |
| Apply layered architecture | `.agents/skills/flutter-apply-architecture-best-practices` |
| Build a responsive/adaptive layout | `.agents/skills/flutter-build-responsive-layout` |
| Fix overflow / unbounded-constraint errors | `.agents/skills/flutter-fix-layout-issues` |
| Wire up declarative routing | `.agents/skills/flutter-setup-declarative-routing` |
| Model JSON (then adapt to Freezed, below) | `.agents/skills/flutter-implement-json-serialization` |
| Add a widget test | `.agents/skills/flutter-add-widget-test` |
| Add an integration test | `.agents/skills/flutter-add-integration-test` |
| Add a widget preview | `.agents/skills/flutter-add-widget-preview` |
| Add localization | `.agents/skills/flutter-setup-localization` |
| Call a REST API | `.agents/skills/flutter-use-http-package` |

The remaining skills (`dart-build-cli-app`, `dart-setup-ffi-assets`, `dart-use-ffigen`,
`dart-migrate-to-checks-package`) are situational — use them only if that exact need
comes up. Run `ls .agents/skills` to see the full set.
...
This repo already uses `very_good_analysis` for lints — keep it. `riverpod_lint` is
wired into `analysis_options.yaml` under the `custom_lint` plugin.
...
- App-lifetime state (e.g. global stores) should be `@Riverpod(keepAlive: true)`;
  ephemeral, screen-scoped state stays auto-dispose.
...
- This app's canonical surface is a **landscape tablet / iPad** (orientation-locked);
  design and golden-test at those breakpoints first.
